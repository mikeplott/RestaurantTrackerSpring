...
public class RestaurantTrackerController {
    ...
    public String create(HttpSession session, String name, String loc, int rating, String comment) throws Exception {
        String username = (String) session.getAttribute("username");
        if (username == null) {
            throw new Exception("Not logged in.");
        }
        User user = users.findByName(username);
        Restaurant r = new Restaurant(name, loc, rating, comment, user);
        restaurants.save(r);
        return "redirect:/";
    }
